      ...
      if (group._id === modalGroupId) {
        const updatedMembers = group.members.map((member, index) => {
          if (index === modalMemberIndex) {
            ...
            if (modalAction === "Deposit Money")
              newAmount = Math.max(newAmount - value, 0);
              ...
              <td className="border border-gray-300 px-4 py-2">
                <table className="w-full border border-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      <th className="border border-gray-200 px-2 py-1 text-sm text-left">
                        #
                      </th>
                      <th className="border border-gray-200 px-2 py-1 text-sm text-left">
                        Name
                      </th>
                      <th className="border border-gray-200 px-2 py-1 text-sm text-center">
                        Amount
                      </th>
                      <th className="border border-gray-200 px-2 py-1 text-sm text-center">
                        Actions
                    ...
                    {group.members.map((member, index) => (
                      <tr key={index} className="text-center">
                        ...
                        <td className="border border-gray-200 px-2 py-1 flex justify-center gap-2">
                          <button
                            onClick={() =>
                              openModal("Add Money", group._id, index)
                            }
                            className="text-blue-500 hover:text-blue-700"
                            title="Add Money"
                          >
                          ...
                          <button
                            onClick={() =>
                              openModal("Deposit Money", group._id, index)
                            }
